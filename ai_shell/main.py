...
from .llm import (
    get_llm_provider, GeminiProvider, LocalLLMProvider,
    ASSISTANT_SYSTEM_PROMPT, METASPLOIT_SYSTEM_PROMPT, WAPITI_SYSTEM_PROMPT,
    extract_command_from_response
)
from .executor import get_executor
from .ui import (
    colors, print_banner, print_mode_selection, print_provider_selection,
    print_local_model_selection, format_error, format_warning, format_info, format_success
)
...
Examples:
  ...
  ai-shell --config myconfig.yaml  # Use custom config file
    ...
    parser.add_argument(
        '--mode', choices=['translator', 'assistant', 'metasploit', 'wapiti'],
        help='Operating mode (default: interactive selection)'
    )
    
    parser.add_argument(
        '--provider', choices=['gemini', 'local'],
...
def interactive_mode_selection() -> str:
    ...
    while True:
        try:
            ...
            mode_map = {'1': 'translator', '2': 'assistant', '3': 'metasploit', '4': 'wapiti'}
    ...
    while True:
        try:
            ...
            if choice in local_models:
                break
            print(format_warning(f"Invalid choice. Please enter {', '.join(local_models.keys())}."))
        except (KeyboardInterrupt, EOFError):
            print("\nExiting...")
            return False
...
def translator_loop():
    ...
    while True:
        try:
            ...
            command_to_run, _ = llm_provider.generate_response(user_prompt, 'translator')
...
async def pty_loop_base(tool_name: str, tool_color: str, system_prompt: str, start_command: list):
    ...
    print(f"To ask the AI for commands, start your prompt with '{colors.PROMPT}?{colors.RESET}'")
